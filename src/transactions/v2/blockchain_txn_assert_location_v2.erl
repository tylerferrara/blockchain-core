...
new(Gateway, Owner, Location, OUI, Nonce) ->
...
-spec new(Gateway :: libp2p_crypto:pubkey_bin(),
          ...
          OUI :: undefined | pos_integer(),
          Nonce :: non_neg_integer()) -> txn_assert_location().
new(Gateway, Owner, Payer, Location, OUI, Nonce) ->
    #blockchain_txn_assert_location_v2_pb{
        gateway=Gateway,
        owner=Owner,
        payer = Payer,
...
-spec oui(txn_assert_location()) -> undefined | pos_integer().
oui(Txn) ->
    Txn#blockchain_txn_assert_location_v2_pb.oui.
...
calculate_fee(Txn, Chain) ->
    ...
    calculate_fee(Txn, Ledger, blockchain_ledger_v2:txn_fees_active(Ledger)).

-spec calculate_fee(txn_assert_location(), blockchain_ledger_v2:ledger(), boolean()) -> non_neg_integer().
...
calculate_fee(Txn, Ledger, true) ->
    case Txn#blockchain_txn_assert_location_v2_pb.payer of
        Payer when Payer == undefined; Payer == <<>> ->
          ...
          ?fee(Txn#blockchain_txn_assert_location_v2_pb{fee=0, staking_fee=0,
...
calculate_staking_fee(Txn, Chain) ->
    Ledger = blockchain:ledger(Chain),
    calculate_staking_fee(Txn, Ledger, blockchain_ledger_v2:txn_fees_active(Ledger)).

-spec calculate_staking_fee(txn_assert_location(), blockchain_ledger_v2:ledger(), boolean()) -> non_neg_integer().
calculate_staking_fee(_Txn, _Ledger, false) ->
    ?LEGACY_STAKING_FEE;
calculate_staking_fee(_Txn, Ledger, true) ->
    blockchain_ledger_v2:staking_fee_txn_assert_location_v2(Ledger).
...
print(#blockchain_txn_assert_location_v2_pb{
        ...
        location = Loc, oui=OUI,
        owner_signature = OS, payer_signature = PS, nonce = Nonce,
        staking_fee = StakingFee, fee = Fee}) ->
    io_lib:format("type=assert_location, gateway=~p, owner=~p, payer=~p, location=~p, oui=~p, owner_signature=~p, payer_signature=~p, nonce=~p, staking_fee=~p, fee=~p",
                  [?TO_ANIMAL_NAME(Gateway), ?TO_B58(Owner), ?TO_B58(Payer),
		   Loc, OUI, OS, PS, Nonce, StakingFee, Fee]).
...
new_test() ->
    Tx = new(),
    ?assertEqual(Tx, new(<<"gateway_address">>, <<"owner_address">>, ?TEST_LOCATION, ?TEST_OUI, 1)).
...
to_json_test() ->
    ...
    ?assert(lists:all(fun(K) -> maps:is_key(K, Json) end,
                      [type, hash, gateway, owner, payer, location, oui, nonce, staking_fee, fee])).
